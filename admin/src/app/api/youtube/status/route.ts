...
type Stream = { id: string; snippet?: { title?: string }; status?: { streamStatus?: string; healthStatus?: { status?: string; configurationIssues?: Array<{ type?: string; severity?: string; reason?: string; description?: string }> } } };
...
export async function GET() {
  ...
  try {
    const [channels, upcoming, active, streams] = await Promise.all([
      ...
      youtubeRequest<ListResponse<Stream>>("/liveStreams?part=id,snippet,status&mine=true&maxResults=25"),
    ]);
